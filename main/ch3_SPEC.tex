...
SPEC algorithm can be summarized as follows. First, inputs are read and some pre-processing of input data is performed. This stage is mainly used to normalize some physics profiles, and setup important flags for the execution. Then, SPEC is built around nested loops, their number depending whether a fixed- or free-boundary calculation is made, and depending on the kind of input physics profile.


\subsection{Inputs}
Different inputs are available to SPEC. They depend whether a fixed- or a free-boundary equilibrium is computer, and which kind of input physics profile is constrained.

In case of a fixed-boundary calculation \citep{hudsonComputationMultiregionRelaxed2012}, the plasma boundary $\Gamma_{PB}$. Free-boundary calculations \citep{Hudson2020c}, on the other hand, require the definition of computational boundary $\Gamma_{CB}$, located between the plasma boundary and the coils. Using standard cylindrical coordinates $\mathbf{x}={R}\mathbf{e}_R + {Z}\mathbf{e}_Z$, the toroidal surfaces are described using double Fourier series,
\begin{align}
	{R}(\theta,\phi) &= \sum_{m=0}^{M_{pol}}\sum_{n=-N_{tor}}^{N_{tor}} R_{mn}\cos(m\theta-nN_{fp}\phi)\\
	{Z}(\theta,\phi) &= \sum_{m=0}^{M_{pol}}\sum_{n=-N_{tor}}^{N_{tor}} Z_{mn}\sin(m\theta-nN_{fp}\phi),
\end{align}
...
This boundary is in general not a magnetic surface, thus $\mathbf{B}\cdot\mathbf{n}\ne 0$, with $\mathbf{n}=\mathbf{e}_\theta\times\mathbf{e}_\phi$ a vector normal to the computational boundary. The magnetic field is the sum of the magnetic field produced by the coils $\mathbf{B}^c$ and by the plasma $\mathbf{B}^p$. To compute the magnetic field inside the vacuum region, the boundary condition $\mathbf{B}\cdot\mathbf{n}$ on $\Gamma_{CB}$ have to be known. The contribution from the coils, $\mathbf{B}^c\cdot\mathbf{n}$ is provided as input. the field produced by the plasma is however unknown, and an initial guess for $\mathbf{B}^p\cdot\mathbf{n}$ has to be provided.  Finally, for free-boundary calculations, the total toroidal current flowing in the plasma $I_{pl}$ and flowing in the coils $I_{coil}$ have to be given, 
\begin{align}
	\mu_0 I_{coil} &= 2\pi\tilde{B}^+_{V,\phi}\\
	\mu_0 I_{pl} &= 2\pi\tilde{B}^-_{V,\theta},
\end{align}
with $\tilde{B}^+_{V,\phi}$ the $m=n=0$ Fourier mode of the covariant toroidal magnetic field on the computational boundary and $\tilde{B}^-_{V,\theta}$ the $m=n=0$ Fourier mode of the covariant poloidal magnetic field on the outer side of the plasma boundary.


Other physics inputs are the number of volumes and the toroidal flux they enclose $\psi_{t,l}$, as well as an initial guess for their interfaces. Then, different kind of profiles can be constrained: either (i) the constant $\mu_l$ and the poloidal flux $\psi_{p,l}$, or (ii) the rotational transform on the inner $\iota_-$ and outer $\iota_+$ interfaces of each volume, or (iii) the net toroidal current flowing in each volume $I^v_{\phi,l}$ and at each interface (current sheet) $I^s_{\phi,l}$. The implementation of the last constraint is the topic of section \ref{sec. current constraint}.
...
\begin{equation}
	M_l = \frac{1}{2}\sum_{m=0}^{M_{pol}}\sum_{n=-N_{tor}}^{N_{tor}} m^\lambda (R_{mn}^2 + Z_{mn}^2),
\end{equation}
...
\begin{equation}
	L_l = \oint\oint\sum_{i=1}^{N_s}\sqrt{[R_l(\theta,\phi)-R_{l-1}(\theta,\phi)]^2 +  [Z_l(\theta,\phi)-Z_{l-1}(\theta,\phi)]^2}d\theta d\phi,
\end{equation}
...
\begin{equation}
	S_l = \frac{1}{2}\oint ({Z}_l(0,\phi) - Z_{l,0})^2 d\phi,
\end{equation}
with
\begin{equation}
...
The target ot minimize is then a linear combination of all angular targets, 
\begin{equation}
